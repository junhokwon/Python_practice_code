...
def get_html_from_url(url,params):
    ...
    # 실제 요청한 url(url과 params를 조합해서 만들어질 요청url을 출력
    # 위의 두 변수 (url,params)는 사용해서는 안됨
...
class Episode:
    ...
    #url,title,rating,date는 프로퍼티구현(전부 읽기전용 ,private으로 선언하지 않음
...
author_name = soup.select_one('div.comicinfo > div.detail > h2 > span.wrt_nm').text.strip()
title = soup.select_one('div.comicinfo > div.detail > h2').contents[0].strip()
#author_name = soup.select_one('div.comicinfo > div.detail > h2 > span > span.wrt_nm').text.strip()
...
for tr in tr_list:
    if not tr.find('td',class_='title'):
        continue
    title = tr.find('td', class_='title').find('a').test
